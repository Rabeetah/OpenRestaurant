...
class ReadyOrders extends React.Component {
  ...
  getAllRestOrders = async () => {
    ...
    var itemorders = [];
    var body = JSON.stringify({ restid: this.state.user.rest_id, status:'Ready'});
    await fetch(
      `http://localhost:4000/customer/order/getallrestordersprepared`,
      {
        method: "POST",
        body,
        headers: {
      ...
      <div>
        {this.state.loading ? (
          ...
          <div class="list-group">
            <h4 style={{ paddingTop: '1.5em' }}>READY ORDERS</h4>
            <hr />
            <div>
                    ...
                    <div style={{ alignContent: 'space-between' }}>
                      <div style={{ display: 'inline' }}>
                        ...
                        <p>
                          ...
                          <button type="button" class="btn btn-primary btn-sm" style={{marginLeft:'2em'}} onClick={()=>this.setOrderStatus(order.orderid)}>Taken</button>
...
export default connect(mapStateToProps, null)(ReadyOrders);