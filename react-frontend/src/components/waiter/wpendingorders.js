...
class PendingOrders extends React.Component {
  ...
  getAllRestOrders = async () => {
    ...
    var itemorders = [];
    var body = JSON.stringify({ restid: this.state.user.rest_id, status:'Ready',payment_method:'cod', delivered: false});
    await fetch(
      `http://localhost:4000/customer/order/waiterreadyorders`,
      {
        method: "POST",
        body,
        headers: {
  ...
  setOrderStatus = async (orderid) => {
    var pointerToThis = this;
    // console.log(id)
    var body = JSON.stringify({ orderid: orderid, restid: this.state.user.rest_id, status: 'Complete'});
    await fetch(
      `http://localhost:4000/customer/order/setdeliveredstatus`,
      {
        method: "POST",
        body,
        headers: {
  ...
  setDeliveredStatus = async (orderid) => {
    var pointerToThis = this;
    // console.log(id)
    var body = JSON.stringify({ orderid: orderid, restid: this.state.user.rest_id, delivered: true, status:'Complete'});
    await fetch(
      `http://localhost:4000/customer/order/setdeliveredstatus`,
      {
        method: "POST",
      ...
      .then((response) => response.json())
      .then((data) => pointerToThis.setState({ norders: data }));
    // await this.setOrderStatus(orderid);
    await this.getAllRestOrders();
  }
      ...
      <div>
        {this.state.loading ? (
          ...
          <div class="list-group">
            <h4 style={{ paddingTop: '1.5em' }}>RECEIVED ORDERS</h4>
            <hr />
            <div>
                    ...
                    <div style={{ alignContent: 'space-between' }}>
                      <div style={{ display: 'inline' }}>
                        ...
                        {/* <p>
                          ...
                          <button type="button" class="btn btn-primary btn-sm" style={{marginLeft:'2em'}} onClick={()=>this.setOrderStatus(order.orderid)}>Complete</button> */}
                        ...
                        <p>
                          ...
                          <button type="button" class="btn btn-primary btn-sm" style={{marginLeft:'2em'}} onClick={()=>this.setDeliveredStatus(order.orderid)}>Delivered</button>
                        </p>
                        <p>
                            ...
                            <div>
                              {item.id == j._id ?
                                <div class="ml-auto justify-content-between" style={{ display: 'inline', alignContent: 'space-between' }}>
                                  ...
                                  <p>
                                    <b style={{paddingRight:'7.75em'}}>Quantity</b>  {item.quantity}
                                  </p>
                                </div>
                                : ''}
                            </div>
